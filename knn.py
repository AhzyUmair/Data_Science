...
xtr,xts,ytr,yts=train_test_split(x,y,random_state=0,test_size=0.25)
...
knn.fit(xtr,ytr)
ypr=knn.predict(xts)
print(np.concatenate((ypr.reshape(len(ypr),1),yts.reshape(len(yts),1)),1))
from sklearn.metrics import confusion_matrix,accuracy_score
cn=confusion_matrix(ypr,yts)
print(cn)
accuracy_score(ypr,yts)
